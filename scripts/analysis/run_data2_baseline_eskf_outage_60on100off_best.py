...
EXP_ID_DEFAULT = "EXP-20260407-data2-baseline-eskf-outage-60on100off-best-r1"
OUTPUT_DIR_DEFAULT = Path("output/data2_baseline_eskf_outage_60on100off_best_r1")
BASE_CONFIG_DEFAULT = Path("config_data2_baseline_eskf_outage_60on100off_best.yaml")
SOLVER_DEFAULT = Path("build/Release/eskf_fusion.exe")
BASELINE_CASE_ID = "data2_baseline_eskf_outage_60on100off_best"
PHASE_NAME_MAP = {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the formal best data2 ESKF outage 60on100off baseline and render its plots.")
...
def run_case(cfg_path: Path, output_dir: Path, case_dir: Path, exe_path: Path) -> dict[str, Any]:
    ...
    if not root_diag.exists():
        raise RuntimeError("missing DIAG.txt after outage best run")
    shutil.copy2(root_diag, diag_path)

    nav_metrics, segment_rows = evaluate_navigation_metrics(cfg_path, sol_path)
    row: dict[str, Any] = {
        "case_id": BASELINE_CASE_ID,
        "case_label": "data2 baseline eskf outage 60on100off best",
...
def write_summary(
    ...
    lines = [
        "# data2 baseline eskf outage 60on100off best summary",
        "",
...
def main() -> None:
    ...
    plot_case = PlotCase(case_id=BASELINE_CASE_ID, label="data2 baseline eskf outage 60on100off best", color="#d62728")
    ...
    plot_state_grid(
        ...
        overview_path,
        "data2 baseline eskf outage 60on100off best all-state overview",
    ...
    plot_state_grid(
        ...
        KEY_COUPLING_STATES,
        key_path,
        "data2 baseline eskf outage 60on100off best key coupling states",
        metadata["phase1_window"][1],
        metadata["phase3_window"][0],
        gnss_off_windows,
        truth_keys_to_hide=plot_config.truth_keys_to_hide,
    ...
    for group_spec in plot_config.group_specs:
        ...
        plot_state_grid(
            ...
            group_path,
            f"data2 baseline eskf outage 60on100off best {selected_group.title}",
            metadata["phase1_window"][1],
            metadata["phase3_window"][0],
            gnss_off_windows,
            plot_mode=plot_mode,
