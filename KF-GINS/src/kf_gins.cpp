...
bool loadConfig(YAML::Node &config, GINSOptions &options) {
    ...
    if (config["gnss_update_debug_path"]) {
        try {
            options.gnss_update_debug_path = config["gnss_update_debug_path"].as<std::string>();
        } catch (YAML::Exception &exception) {
            std::cout << "Failed when loading configuration. Please check gnss_update_debug_path!" << std::endl;
            return false;
        }
    }
    if (config["predict_debug_path"]) {
        try {
            options.predict_debug_path = config["predict_debug_path"].as<std::string>();
        } catch (YAML::Exception &exception) {
            std::cout << "Failed when loading configuration. Please check predict_debug_path!" << std::endl;
            return false;
        }
    }
    if (config["predict_debug_start_time"]) {
        try {
            options.predict_debug_start_time = config["predict_debug_start_time"].as<double>();
        } catch (YAML::Exception &exception) {
            std::cout << "Failed when loading configuration. Please check predict_debug_start_time!" << std::endl;
            return false;
        }
    }
    if (config["predict_debug_end_time"]) {
        try {
            options.predict_debug_end_time = config["predict_debug_end_time"].as<double>();
        } catch (YAML::Exception &exception) {
            std::cout << "Failed when loading configuration. Please check predict_debug_end_time!" << std::endl;
            return false;
        }
    }
