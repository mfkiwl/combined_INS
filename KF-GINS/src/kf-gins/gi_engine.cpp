...
std::string VectorToCsvField(const Eigen::VectorXd &vec) {
    ...
    for (Eigen::Index i = 0; i < vec.size(); ++i) {
        ...
        oss << std::setprecision(12) << vec(i);
    }
    oss << "]";
    return oss.str();
}

std::string MatrixToCsvField(const Eigen::MatrixXd &mat) {
    std::ostringstream oss;
    oss << "[";
    for (Eigen::Index r = 0; r < mat.rows(); ++r) {
        for (Eigen::Index c = 0; c < mat.cols(); ++c) {
            ...
            oss << std::setprecision(12) << mat(r, c);
        }
    }
    oss << "]";
    return oss.str();
...
Eigen::MatrixXd ReorderToCommonPredictStateOrder(const Eigen::MatrixXd &mat) {
    static const int kOrder[21] = {
        0, 1, 2, 3, 4, 5, 6, 7, 8,
        12, 13, 14,
        9, 10, 11,
        15, 16, 17, 18, 19, 20,
    };
    Eigen::MatrixXd reordered(21, 21);
    for (int r = 0; r < 21; ++r) {
        for (int c = 0; c < 21; ++c) {
            reordered(r, c) = mat(kOrder[r], kOrder[c]);
        }
    }
    return reordered;
}
...
GIEngine::GIEngine(GINSOptions &options) {
    ...
    if (!options_.gnss_update_debug_path.empty()) {
        gnss_update_debug_file_.open(options_.gnss_update_debug_path, std::ios::out | std::ios::trunc);
        if (gnss_update_debug_file_.is_open()) {
            gnss_update_debug_file_ << std::fixed << std::setprecision(9);
            gnss_update_debug_file_
                << "update_index,tag,gnss_t,state_t,"
                << "y_x,y_y,y_z,y_norm,"
                ...
                << "prior_cov_ba_pos_mat,prior_cov_ba_att_mat,"
                << "r_mat,"
                << "s_mat,"
                ...
                << "state_pos_before_x,state_pos_before_y,state_pos_before_z,"
                << "state_vel_before_x,state_vel_before_y,state_vel_before_z,"
                << "state_att_before_roll,state_att_before_pitch,state_att_before_yaw,"
                << "bg_before_x,bg_before_y,bg_before_z,"
                << "ba_before_x,ba_before_y,ba_before_z,"
                << "sg_before_x,sg_before_y,sg_before_z,"
                << "sa_before_x,sa_before_y,sa_before_z,"
                << "state_pos_after_x,state_pos_after_y,state_pos_after_z,"
                << "state_vel_after_x,state_vel_after_y,state_vel_after_z,"
                << "state_att_after_roll,state_att_after_pitch,state_att_after_yaw,"
                << "bg_after_x,bg_after_y,bg_after_z,"
                << "ba_after_x,ba_after_y,ba_after_z,"
                << "sg_after_x,sg_after_y,sg_after_z,"
                << "sa_after_x,sa_after_y,sa_after_z\n";
        }
    }

    if (!options_.predict_debug_path.empty()) {
        predict_debug_file_.open(options_.predict_debug_path, std::ios::out | std::ios::trunc);
        if (predict_debug_file_.is_open()) {
            predict_debug_file_ << std::fixed << std::setprecision(9);
            predict_debug_file_
                << "step_index,tag,t_prev,t_curr,dt,"
                << "p_before_common_mat,phi_common_mat,qd_common_mat,"
                << "phi_p_common_mat,p_after_common_mat\n";
        }
    }
}
...
void GIEngine::insPropagation(IMU &imupre, IMU &imucur) {
    ...
    const Eigen::MatrixXd CovBefore = Cov_;
    ...
    const Eigen::MatrixXd PhiP = Phi * CovBefore * Phi.transpose();

    // EKF预测传播系统协方差和系统误差状态
    // do EKF predict to propagate covariance and error state
    EKFPredict(Phi, Qd);
    writePredictDebugRow("ins", imupre, imucur, Phi, Qd, CovBefore, PhiP, Cov_);
}
...
void GIEngine::gnssUpdate(GNSS &gnssdata) {
    ...
    pending_gnss_update_debug_.valid = false;
    if (gnss_update_debug_file_.is_open()) {
        pending_gnss_update_debug_.valid = true;
        pending_gnss_update_debug_.update_index = gnss_update_debug_count_;
        pending_gnss_update_debug_.gnss_t = gnssdata.time;
        pending_gnss_update_debug_.state_t = timestamp_;
        pending_gnss_update_debug_.innovation = dz.block<3, 1>(0, 0);
        pending_gnss_update_debug_.prior_std_ba =
            Cov_.block<3, 3>(BA_ID, BA_ID).diagonal().cwiseMax(0.0).cwiseSqrt();
        pending_gnss_update_debug_.prior_cov_pos = Cov_.block<3, 3>(P_ID, P_ID);
        pending_gnss_update_debug_.prior_cov_vel = Cov_.block<3, 3>(V_ID, V_ID);
        pending_gnss_update_debug_.prior_cov_att = Cov_.block<3, 3>(PHI_ID, PHI_ID);
        pending_gnss_update_debug_.prior_cov_pos_vel = Cov_.block<3, 3>(P_ID, V_ID);
        pending_gnss_update_debug_.prior_cov_pos_att = Cov_.block<3, 3>(P_ID, PHI_ID);
        pending_gnss_update_debug_.prior_cov_vel_att = Cov_.block<3, 3>(V_ID, PHI_ID);
        pending_gnss_update_debug_.prior_cov_ba_pos = Cov_.block<3, 3>(BA_ID, P_ID);
        pending_gnss_update_debug_.prior_cov_ba_att = Cov_.block<3, 3>(BA_ID, PHI_ID);
        pending_gnss_update_debug_.r_mat = R_gnsspos.block<3, 3>(0, 0);
        pending_gnss_update_debug_.h_att = H_gnsspos.block<3, 3>(0, PHI_ID);
        pending_gnss_update_debug_.pva_before = pvacur_;
        pending_gnss_update_debug_.imuerror_before = imuerror_;
    }
    EKFUpdate(dz, H_gnsspos, R_gnsspos);
    if (pending_gnss_update_debug_.valid) {
        pending_gnss_update_debug_.dx = dx_;
    }
...
void GIEngine::EKFUpdate(Eigen::MatrixXd &dz, Eigen::MatrixXd &H, Eigen::MatrixXd &R) {
    ...
    if (pending_gnss_update_debug_.valid && temp.rows() >= 3 && temp.cols() >= 3) {
        pending_gnss_update_debug_.s_mat = temp.block<3, 3>(0, 0);
    }
    Eigen::MatrixXd K = Cov_ * H.transpose() * temp.inverse();
    if (pending_gnss_update_debug_.valid && K.rows() >= P_ID + 3 && K.cols() >= 3) {
        pending_gnss_update_debug_.k_pos = K.block<3, 3>(P_ID, 0);
    }
    if (pending_gnss_update_debug_.valid && K.rows() >= V_ID + 3 && K.cols() >= 3) {
        pending_gnss_update_debug_.k_vel = K.block<3, 3>(V_ID, 0);
    }
    if (pending_gnss_update_debug_.valid && K.rows() >= PHI_ID + 3 && K.cols() >= 3) {
        pending_gnss_update_debug_.k_att = K.block<3, 3>(PHI_ID, 0);
    }
    if (pending_gnss_update_debug_.valid && K.rows() >= BA_ID + 3 && K.cols() >= 3) {
        pending_gnss_update_debug_.k_ba = K.block<3, 3>(BA_ID, 0);
    }
...
void GIEngine::writeGnssUpdateDebugRow() {

    if (!pending_gnss_update_debug_.valid || !gnss_update_debug_file_.is_open()) {
        pending_gnss_update_debug_.valid = false;
        return;
    }

    const auto &record = pending_gnss_update_debug_;
    gnss_update_debug_file_
        << record.update_index << ",GNSS_POS,"
        << record.gnss_t << "," << record.state_t << ","
        << record.innovation.x() << "," << record.innovation.y() << "," << record.innovation.z() << ","
        << record.innovation.norm() << ","
        << record.dx(P_ID + 0) << "," << record.dx(P_ID + 1) << "," << record.dx(P_ID + 2) << ","
        << record.dx(V_ID + 0) << "," << record.dx(V_ID + 1) << "," << record.dx(V_ID + 2) << ","
        << record.dx(PHI_ID + 0) << "," << record.dx(PHI_ID + 1) << "," << record.dx(PHI_ID + 2) << ","
        << record.dx(BA_ID + 0) << "," << record.dx(BA_ID + 1) << "," << record.dx(BA_ID + 2) << ","
        << record.dx(BG_ID + 0) << "," << record.dx(BG_ID + 1) << "," << record.dx(BG_ID + 2) << ","
        << record.dx(SG_ID + 0) << "," << record.dx(SG_ID + 1) << "," << record.dx(SG_ID + 2) << ","
        << record.dx(SA_ID + 0) << "," << record.dx(SA_ID + 1) << "," << record.dx(SA_ID + 2) << ","
        << VectorToCsvField(record.prior_std_ba) << ","
        << MatrixToCsvField(record.prior_cov_pos) << ","
        << MatrixToCsvField(record.prior_cov_vel) << ","
        << MatrixToCsvField(record.prior_cov_att) << ","
        << MatrixToCsvField(record.prior_cov_pos_vel) << ","
        << MatrixToCsvField(record.prior_cov_pos_att) << ","
        << MatrixToCsvField(record.prior_cov_vel_att) << ","
        << MatrixToCsvField(record.prior_cov_ba_pos) << ","
        << MatrixToCsvField(record.prior_cov_ba_att) << ","
        << MatrixToCsvField(record.r_mat) << ","
        << MatrixToCsvField(record.s_mat) << ","
        << VectorToCsvField(record.h_att.col(0)) << ","
        << VectorToCsvField(record.h_att.col(1)) << ","
        << VectorToCsvField(record.h_att.col(2)) << ","
        << VectorToCsvField(record.k_pos.row(0).transpose()) << ","
        << VectorToCsvField(record.k_pos.row(1).transpose()) << ","
        << VectorToCsvField(record.k_pos.row(2).transpose()) << ","
        << VectorToCsvField(record.k_vel.row(0).transpose()) << ","
        << VectorToCsvField(record.k_vel.row(1).transpose()) << ","
        << VectorToCsvField(record.k_vel.row(2).transpose()) << ","
        << VectorToCsvField(record.k_att.row(0).transpose()) << ","
        << VectorToCsvField(record.k_att.row(1).transpose()) << ","
        << VectorToCsvField(record.k_att.row(2).transpose()) << ","
        << VectorToCsvField(record.k_ba.row(0).transpose()) << ","
        << VectorToCsvField(record.k_ba.row(1).transpose()) << ","
        << VectorToCsvField(record.k_ba.row(2).transpose()) << ","
        << record.pva_before.pos.x() << "," << record.pva_before.pos.y() << "," << record.pva_before.pos.z() << ","
        << record.pva_before.vel.x() << "," << record.pva_before.vel.y() << "," << record.pva_before.vel.z() << ","
        << record.pva_before.att.euler.x() << "," << record.pva_before.att.euler.y() << "," << record.pva_before.att.euler.z() << ","
        << record.imuerror_before.gyrbias.x() << "," << record.imuerror_before.gyrbias.y() << "," << record.imuerror_before.gyrbias.z() << ","
        << record.imuerror_before.accbias.x() << "," << record.imuerror_before.accbias.y() << "," << record.imuerror_before.accbias.z() << ","
        << record.imuerror_before.gyrscale.x() << "," << record.imuerror_before.gyrscale.y() << "," << record.imuerror_before.gyrscale.z() << ","
        << record.imuerror_before.accscale.x() << "," << record.imuerror_before.accscale.y() << "," << record.imuerror_before.accscale.z() << ","
        << pvacur_.pos.x() << "," << pvacur_.pos.y() << "," << pvacur_.pos.z() << ","
        << pvacur_.vel.x() << "," << pvacur_.vel.y() << "," << pvacur_.vel.z() << ","
        << pvacur_.att.euler.x() << "," << pvacur_.att.euler.y() << "," << pvacur_.att.euler.z() << ","
        << imuerror_.gyrbias.x() << "," << imuerror_.gyrbias.y() << "," << imuerror_.gyrbias.z() << ","
        << imuerror_.accbias.x() << "," << imuerror_.accbias.y() << "," << imuerror_.accbias.z() << ","
        << imuerror_.gyrscale.x() << "," << imuerror_.gyrscale.y() << "," << imuerror_.gyrscale.z() << ","
        << imuerror_.accscale.x() << "," << imuerror_.accscale.y() << "," << imuerror_.accscale.z() << "\n";

    pending_gnss_update_debug_.valid = false;
    ++gnss_update_debug_count_;
}

void GIEngine::writePredictDebugRow(const std::string &tag,
                                    const IMU &imupre,
                                    const IMU &imucur,
                                    const Eigen::MatrixXd &Phi,
                                    const Eigen::MatrixXd &Qd,
                                    const Eigen::MatrixXd &CovBefore,
                                    const Eigen::MatrixXd &PhiP,
                                    const Eigen::MatrixXd &CovAfter) {

    if (!predict_debug_file_.is_open()) {
        return;
    }
    const double start_t = std::isfinite(options_.predict_debug_start_time)
                               ? options_.predict_debug_start_time
                               : -std::numeric_limits<double>::infinity();
    const double end_t = std::isfinite(options_.predict_debug_end_time)
                             ? options_.predict_debug_end_time
                             : std::numeric_limits<double>::infinity();
    // Keep debug rows strictly inside the requested predict window so the
    // compare tooling does not align an extra pre-window step.
    constexpr double kPredictWindowTol = 1e-9;
    if (imupre.time < start_t - kPredictWindowTol ||
        imupre.time >= end_t - kPredictWindowTol) {
        return;
    }

    predict_debug_file_
        << predict_debug_count_++ << ","
        << tag << ","
        << imupre.time << ","
        << imucur.time << ","
        << imucur.dt << ","
        << MatrixToCsvField(ReorderToCommonPredictStateOrder(CovBefore)) << ","
        << MatrixToCsvField(ReorderToCommonPredictStateOrder(Phi)) << ","
        << MatrixToCsvField(ReorderToCommonPredictStateOrder(Qd)) << ","
        << MatrixToCsvField(ReorderToCommonPredictStateOrder(PhiP)) << ","
        << MatrixToCsvField(ReorderToCommonPredictStateOrder(CovAfter)) << "\n";
}
